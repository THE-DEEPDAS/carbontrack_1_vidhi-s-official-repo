...
const Sidebar: React.FC<SidebarProps> = ({ userData, sidebarCollapsed, setSidebarCollapsed }) => {
    ...
    >
      <div className="p-4 border-b border-gray-200">
        <div className="flex flex-col items-center">
          ...
          {!sidebarCollapsed && (
            <>
              <h3 className="mt-2 font-semibold text-black">{userData?.fullName || 'Jane Smith'}</h3>
      ...
      <div className="p-4">
        {!sidebarCollapsed && <h4 className="text-xs font-semibold text-gray-500 uppercase mb-2">Navigation</h4>}
        <nav className="space-y-1">
          {navLinks.map((link, index) => (
            ...
            >
              ...
              {!sidebarCollapsed && location.pathname === link.path && <ChevronRight className="h-4 w-4" />}
            </Link>
          ))}
        </nav>
      </div>
